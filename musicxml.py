...
def to_element_case(name: str) -> str:
...
class MusicElementBase(metaclass=AutoAlias):
  ...
  def __init__(self, *args, **kwargs):
    self.element = ET.Element(to_element_case(self.__class__.__name__))
    ...
    for k, v_args in kwargs.items():
      ...
      if k == k.lower():
        ...
        if namespace:
          namespace = namespace + ":"
        self.element.set(f"{namespace}{k.replace("_", "-")}", str(v_args))
        continue
  ...
  def __enter__(self):
    ...
    for cls in inspect.getmro(self.__class__):
      for name, attr_value in cls.__dict__.items():
        # Check 1: Is attr_value a class and a subclass of MusicElementBase?
        # (This covers inner classes that are defined as subclasses of MusicElementBase)
        ...
        # Check 2: Is attr_value an instance of MusicElementBase?
        # (This covers class attributes that are instances, e.g., constants)
        elif isinstance(attr_value, MusicElementBase):
          if name not in caller_frame.f_locals:
            caller_frame.f_locals[name] = attr_value  # Inject the instance
            self._added_to_caller_locals[name] = attr_value
    return self
