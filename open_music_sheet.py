...
@st.cache_data
def list_sheet(dir = ""):
  url = "https://api.github.com/repos/yufanyufan/open_music_sheet/contents" + dir
  response = requests.get(url, headers={"Authorization": st.secrests["github_key"]})
  response.raise_for_status()  # Raise an exception for bad status codes
  data = response.json()
  return [item['name'] + "/"if item['type'] == 'dir' else item['name'] for item in data
...
def render_path_chooser(path: str) -> str:
  with next(cols):
    ...
    if sheet_list:
      choosen = st.selectbox("Select a File", index = None, options=sheet_list, key=path, label_visibility="collapsed")
      if choosen:
        if choosen.endswith("/"):
...
def get_music_sheet(path: str) -> str:
  url = "https://raw.githubusercontent.com/yufanyufan/open_music_sheet/main/" + path
